...
import tudbut.mod.client.crimson.mods.Debug;
...
public class ChatUtils { // Everything here is kinda self-explanatory
    ...
    public static void simulateSend(String msg, boolean addToHistory) {
        ...
        if (addToHistory) {
            CRIMSON.mc.ingameGUI.getChatGUI().addToSentMessages(msg);
        }
        if (net.minecraftforge.client.ClientCommandHandler.instance.executeCommand(CRIMSON.mc.player, msg) != 0)
            return;
        CRIMSON.mc.player.sendChatMessage(msg);
    }
}
