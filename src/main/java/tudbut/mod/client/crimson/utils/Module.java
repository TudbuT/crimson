...
public abstract class Module {
    ...
    public ArrayList<GuiCRIMSON.Button> subButtons = new ArrayList<>();
    public Map<String, KeyBind> customKeyBinds = new HashMap<>();
    private GuiCRIMSON.Button[] confirmationButtons = new GuiCRIMSON.Button[3];
    
    {
        confirmationButtons[0] = new GuiCRIMSON.Button("Are you sure?", text -> {});
        confirmationButtons[1] = new GuiCRIMSON.Button("Yes", text -> {
            displayConfirmation = false;
            onConfirm(true);
        });
        confirmationButtons[2] = new GuiCRIMSON.Button("No", text -> {
            displayConfirmation = false;
            onConfirm(false);
        });
    }
    private GuiCRIMSON.Button keyButton = Setting.createKey("KeyBind: $val", key);
    ...
    public final GuiCRIMSON.Button[] getSubButtons() {
        if(displayConfirmation)
            return confirmationButtons;
        ArrayList<GuiCRIMSON.Button> buttons = (ArrayList<GuiCRIMSON.Button>) subButtons.clone();
        buttons.add(keyButton);
        return buttons.toArray(new GuiCRIMSON.Button[0]);
    }
    ...
    // used in the CRIMSON class for saving the config as file, not intended for overriding,
    ...
    public static class KeyBind {
        ...
        public void onTick() {
            if(key != null && CRIMSON.mc.currentScreen == null) {
