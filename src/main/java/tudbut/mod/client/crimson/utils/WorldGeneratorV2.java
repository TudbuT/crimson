...
public class WorldGeneratorV2 extends MinecraftServer
{
    ...
    public static WorldGeneratorV2 create(WorldSettings settings) {
        YggdrasilAuthenticationService as = new YggdrasilAuthenticationService(CRIMSON.mc.getProxy(), UUID.randomUUID().toString());
        YggdrasilGameProfileRepository gpr = new YggdrasilGameProfileRepository(as);
        return new WorldGeneratorV2(CRIMSON.mc, settings, as, as.createMinecraftSessionService(), gpr, new PlayerProfileCache(gpr, new File("ttc/usercache.json")));
    }
    ...
    public void tick()
    {
        ...
        if (this.isGamePaused)
        {
            ...
            if (CRIMSON.mc.gameSettings.renderDistanceChunks != this.getPlayerList().getViewDistance()) {
                LOGGER.info("Changing view distance to {}, from {}", CRIMSON.mc.gameSettings.renderDistanceChunks, this.getPlayerList().getViewDistance());
                this.getPlayerList().setViewDistance(CRIMSON.mc.gameSettings.renderDistanceChunks);
            }
    
            WorldInfo worldinfo1 = this.worlds[0].getWorldInfo();
            WorldInfo worldinfo = CRIMSON.mc.world.getWorldInfo();
    ...
    {
        return CRIMSON.mc.world.getWorldInfo().getDifficulty();
    }
