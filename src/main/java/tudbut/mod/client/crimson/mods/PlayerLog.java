...
public class PlayerLog extends Module {
    ...
    {
        subButtons.add(new GuiCRIMSON.Button("Reset logout spots", text -> {
            logouts.clear();
        }));
    }
    ...
    public void onSubTick() {
        // Is online?
        if (CRIMSON.mc.getConnection() == null)
            return;
        
        if (playersLastTick == null) {
            playersLastTick = CRIMSON.mc.getConnection().getPlayerInfoMap().toArray(new NetworkPlayerInfo[0]);
        }
        EntityPlayer[] visiblePlayersThisTick = CRIMSON.mc.world.playerEntities.toArray(new EntityPlayer[0]);
        NetworkPlayerInfo[] playersThisTick = CRIMSON.mc.getConnection().getPlayerInfoMap().toArray(new NetworkPlayerInfo[0]);
