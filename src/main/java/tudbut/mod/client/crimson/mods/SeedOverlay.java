...
public class SeedOverlay extends Module {
    ...
    public void updateButtons() {
        subButtons.clear();
        subButtons.add(new GuiCRIMSON.Button("Update world data", text -> ThreadManager.run(() -> {
            world = CRIMSON.world;
            worldOptions = CRIMSON.world.getWorldInfo().getGeneratorOptions();
            worldType = CRIMSON.world.getWorldType();
            ...
            if(seed != Long.MAX_VALUE) {
                ThreadManager.run(() -> {
                    ...
                    world = generator.getWorld(CRIMSON.world.provider.getDimension());
                });
            }
            else
                ChatUtils.print("Error: No seed given! ',seedoverlay <seed>' to set");
        })));
        subButtons.add(new GuiCRIMSON.Button("Delete world data", text -> ThreadManager.run(() -> {
            world = null;
            generator.stopServer();
            generator = null;
        })));
        subButtons.add(new GuiCRIMSON.Button("Render type: " + (renderType == 0 ? "Box" : (renderType == 1 ? "Marker" : "Plane")), text -> {
            renderType++;
            if(renderType > 2)
                renderType = 0;
    ...
    private void update() {
        Map<BlockPos, Integer> toRender = new HashMap<>();
        EntityPlayer player = CRIMSON.player;
            ...
            for (int z = -8 * 16; z < 8 * 16; z++) {
                for (int x = -8 * 16; x < 8 * 16; x++) {
                    ...
                    for (int y = 0; y < 256; y++) {
                        ...
                        if (CRIMSON.mc.world.isBlockLoaded(bp, false) && world.getChunkFromBlockCoords(bp).isTerrainPopulated()) {
                            //ChatUtils.print("Check!");
                            IBlockState a = CRIMSON.world.getBlockState(bp);
    ...
    public void onRenderWorld(Event event) {
        ...
        if(event instanceof RenderWorldLastEvent)
            if(this.enabled && CRIMSON.isIngame()) {
