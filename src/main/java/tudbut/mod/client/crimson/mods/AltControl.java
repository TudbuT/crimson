...
import tudbut.mod.client.crimson.utils.*;
...
import static tudbut.mod.client.crimson.utils.CRIMSONIC.*;
...
public class AltControl extends Module {
    ...
    {
        customKeyBinds.put("kill", new KeyBind(null, () -> CRIMSON.mc.displayGuiScreen(
                new GuiPlayerSelect(
        ...
        customKeyBinds.put("follow", new KeyBind(null, () -> CRIMSON.mc.displayGuiScreen(
                new GuiPlayerSelect(
                        CRIMSON.world.playerEntities.toArray(new EntityPlayer[0]),
                        player -> {
                            if (server != null)
    ...
    private void updateButtons() {
        ...
        if(mode == -1) {
            subButtons.add(new GuiCRIMSON.Button("Main mode", text -> {
                if (mode != -1)
                    return;
            ...
            subButtons.add(new GuiCRIMSON.Button("Alt mode", text -> {
                if (mode != -1)
                    return;
            ...
            subButtons.add(new GuiCRIMSON.Button("End connection", text ->
                    onChat("end", "end".split(" "))));
            subButtons.add(new GuiCRIMSON.Button("List", text ->
                    onChat("list", "list".split(" "))));
        }
        if(mode == 0) {
            subButtons.add(new GuiCRIMSON.Button("TPA alts here", text ->
                    onChat("tpa", "tpa".split(" "))));
            subButtons.add(new GuiCRIMSON.Button("Stop alts", text ->
                    onChat("stop", "stop".split(" "))));
            subButtons.add(new GuiCRIMSON.Button("Follow me", text ->
                    onChat("follow", "follow".split(" "))));
            subButtons.add(new GuiCRIMSON.Button("Send client config", text ->
                    onChat("send", "send".split(" "))));
            subButtons.add(new GuiCRIMSON.Button("Use elytra: " + useElytra, text -> {
                onChat("telytra", "telytra".split(" "));
                text.set("Use elytra: " + useElytra);
            }));
            subButtons.add(new GuiCRIMSON.Button("Bot main: " + botMain, text -> {
                botMain = !botMain;
    ...
    public void onTick() {
        if(useElytra && !stopped) {
            if(CRIMSON.isIngame()) {
                NetworkPlayerInfo[] players = Objects.requireNonNull(CRIMSON.mc.getConnection()).getPlayerInfoMap().toArray(new NetworkPlayerInfo[0]);
                
                if (main.uuid.equals(CRIMSON.player.getUniqueID())) {
                ...
                // Target is in rd
                if (commonTargetPlayer != null && CRIMSON.world.getPlayerEntityByName(commonTargetPlayer.getName()) != null)
                    follow();
                // Target is not in rd, but isnt stopped
                    ...
                    if (!main.uuid.equals(CRIMSON.player.getUniqueID())) {
                        ...
                        if (
                                CRIMSON.world.getPlayerEntityByName(main.name) == null &&
    ...
    public void onPacketSC(PacketSC packet) {
        ...
        try {
            ...
            switch (packet.type()) {
                case INIT:
                    main = new Alt();
                    sendPacket(PacketsCS.NAME, CRIMSON.mc.getSession().getProfile().getName());
                    break;
                case NAME:
                    main.name = packet.content();
                    ChatUtils.print("§4[CRIMSON] §rConnection to main " + main.name + " established!");
                    sendPacket(PacketsCS.UUID, CRIMSON.mc.getSession().getProfile().getId().toString());
                    break;
                case UUID:
                    main.uuid = UUID.fromString(packet.content());
                    ChatUtils.print("§4[CRIMSON] §rGot UUID from main " + main.name + ": " + packet.content());
                    sendPacket(PacketsCS.KEEPALIVE, "");
                    break;
                case TPA:
                    ChatUtils.print("§4[CRIMSON] §rTPA'ing main account...");
                    CRIMSON.player.sendChatMessage("/tpa " + main.name);
                    break;
                case EXECUTE:
                    ChatUtils.print("§4[CRIMSON] §rSending message received from main account...");
                    ChatUtils.simulateSend(packet.content(), false);
                    break;
                case LIST:
                    CRIMSON.logger.info("§4[CRIMSON] §rReceived alt list from main.");
                    ...
                    break;
                case KILL:
                    ChatUtils.print("§4[CRIMSON] §rKilling player " + packet.content());
                    kill(packet.content());
                    break;
                case FOLLOW:
                    ChatUtils.print("§4[CRIMSON] §rFollowing " + packet.content());
                ...
                case STOP:
                    ...
                    break;
                case CONFIG:
                    CRIMSON.cfg = Utils.stringToMap(packet.content());
    ...
    public void onPacketCS(PacketCS packet, PBIC.Connection connection) throws PBIC.PBICException.PBICWriteException {
        ...
        switch (packet.type()) {
            case NAME:
                ...
                ChatUtils.print("§4[CRIMSON] §rConnection to alt " + packet.content() + " established!");
                connection.writePacket(getPacketSC(PacketsSC.NAME, CRIMSON.mc.getSession().getProfile().getName()));
                break;
            case UUID:
                altsMap.get(connection).uuid = UUID.fromString(packet.content());
                ChatUtils.print("§4[CRIMSON] §rGot UUID from alt " + altsMap.get(connection).name + ": " + packet.content());
                connection.writePacket(getPacketSC(PacketsSC.UUID, CRIMSON.mc.getSession().getProfile().getId().toString()));
            ...
            case LOST:
                ...
                if(player != null && CRIMSON.world != null) {
    ...
    public void onChat(String s, String[] args) {
        try {
            if (s.equals("server") && server == null) {
                main = new Alt();
                main.name = CRIMSON.mc.getSession().getProfile().getName();
                main.uuid = CRIMSON.mc.getSession().getProfile().getId();
                ...
                ChatUtils.print("§4[CRIMSON] §aServer started");
            }
            if (args[0].equals("client") && client == null) {
                ...
                ChatUtils.print("§4[CRIMSON] §aClient started");
                ThreadManager.run("CRIMSONIC client receive thread", () -> {
                    while (true) {
                        String string = "UNKNOWN";
                        try {
            ...
            if(args.length >= 2) {
                ...
                if(args[0].equals("stop") && s.contains(" ")) {
                    ...
                    ChatUtils.print("§4[CRIMSON] §rStopping killing player " + st);
                    if(botMain) {
                        stop(st);
                    }
                }
            ...
            if (s.equals("stop")) {
                ...
                ChatUtils.print("§4[CRIMSON] §rStopping killing/following all players");
                if(botMain) {
                    stop(null);
                }
            }
            ...
            if (s.equals("send")) {
                CRIMSON.getInstance().setConfig();
                sendPacketSC(PacketsSC.CONFIG, Utils.mapToString(CRIMSON.cfg));
                ChatUtils.print("§4[CRIMSON] §rSending config to all alts");
            }
    ...
    public void follow(String name) {
        if(CRIMSON.player.getName().equals(name))
            return;
        commonTargetPlayer = CRIMSON.world.getPlayerEntityByName(name);
        follow();
    }
    ...
    public void stop(String name) {
        ...
        if(name == null || name.equals("")) {
            ...
            if (aura.targets.size() != 0) {
                ChatUtils.print("§4[CRIMSON] §rKilling player " + name);
