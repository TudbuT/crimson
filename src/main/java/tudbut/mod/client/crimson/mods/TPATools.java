...
public class TPATools extends Module {
    ...
    {
        subButtons.add(new GuiCRIMSON.Button("Send /tpa to everyone", text -> {
            onChat("tpa", null);
        }));
        subButtons.add(new GuiCRIMSON.Button("Send /tpahere to everyone", text -> {
            onChat("tpahere", null);
        }));
        ...
        subButtons.add(new GuiCRIMSON.Button("Stop", text -> {
            stop = true;
            CRIMSON.player.sendChatMessage("/tpacancel");
        ...
        if (s.equalsIgnoreCase("tpa")) {
            ...
            ThreadManager.run(() -> {
                ...
                for (NetworkPlayerInfo info : Objects.requireNonNull(CRIMSON.mc.getConnection()).getPlayerInfoMap().toArray(new NetworkPlayerInfo[0])) {
                    ...
                    try {
                        // Send /tpa <player>
                        CRIMSON.mc.player.sendChatMessage("/tpa " + info.getGameProfile().getName());
                        // Notify the user
