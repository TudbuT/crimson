...
public class PopCount extends Module {
    ...
    public void updateBinds() {
        subButtons.clear();
        subButtons.add(new GuiCRIMSON.Button("Reset", text -> {
            counters = new HashMap<>();
        }));
        subButtons.add(new GuiCRIMSON.Button("CountOwn: " + countOwn, text -> {
            countOwn = !countOwn;
    ...
    public void onTick() {
        ...
        EntityPlayer[] visiblePlayers = CRIMSON.world.playerEntities.toArray(new EntityPlayer[0]);
    
        EntityPlayer[] players = counters.keySet().toArray(new EntityPlayer[0]);
        for (int i = 0; i < visiblePlayers.length; i++) {
            if(countOwn || visiblePlayers[i].getEntityId() != CRIMSON.player.getEntityId()) {
                boolean b = false;
