...
public class AutoConfig extends Module {
    ...
    public void updateButtons() {
        subButtons.clear();
        subButtons.add(new GuiCRIMSON.Button("Mode: " + (mode ? "Server" : "Custom"), text -> {
            mode = !mode;
            text.set("Mode: " + (mode ? "Server" : "Custom"));
            updateButtons();
        }));
        if (mode) {
            subButtons.add(new GuiCRIMSON.Button("Server: " + server.name, text -> {
            ...
            subButtons.add(new GuiCRIMSON.Button("Has stacked totems: " + stackedTots, text -> {
                stackedTots = !stackedTots;
                text.set("Has stacked totems: " + stackedTots);
            }));
            subButtons.add(new GuiCRIMSON.Button("PvP meta: " + (pvp ? "32k" : "Crystal"), text -> {
                pvp = !pvp;
                text.set("PvP meta: " + (pvp ? "32k" : "Crystal"));
            }));
            subButtons.add(new GuiCRIMSON.Button("Has /tpa: " + tpa, text -> {
                tpa = !tpa;
                text.set("Has /tpa: " + tpa);
            }));
        }
        subButtons.add(new GuiCRIMSON.Button("Set", text -> {
            // Confirm set
            if (mode) {
