...
import tudbut.mod.client.crimson.mods.ClickGUI;
...
public class GuiCRIMSON extends GuiScreen {
    ...
    private void resetButtons() {
        ...
        for (int i = 0, j = 0; i < CRIMSON.modules.length; i++) {
            int x = j / 15;
            int y = j - x * 15;
            
            // Don't add the button if it isn't requested
            if (!CRIMSON.modules[i].displayOnClickGUI())
                continue;
            
            // Create the button
            int r = i;
            Button b = new Button(
                    10 + (205 * x), 10 + (y * 25), CRIMSON.modules[r].getClass().getSimpleName() + ": " + CRIMSON.modules[r].enabled,
                    (text) -> {
                        if (CRIMSON.modules[r].enabled = !CRIMSON.modules[r].enabled)
                            CRIMSON.modules[r].onEnable();
                        else
                            CRIMSON.modules[r].onDisable();
    ...
    private void updateButtons() {
        ...
        for (int i = 0; i < CRIMSON.modules.length; i++) {
            if (buttons[i] != null)
                buttons[i].text.set(CRIMSON.modules[i].getClass().getSimpleName() + ": " + CRIMSON.modules[i].enabled);
        }
    }
    ...
    public static class Button {
        ...
        // Constructor used by GuiCRIMSON to construct a button with an associated module
        ...
        // Render the button
        public void draw(GuiCRIMSON gui) {
            if (!display)
                return;
            ...
            drawRect(x, y, x + 200, y + ySize(), color);
        ...
        public boolean mouseClicked(int clickX, int clickY, int button) {
            if (clickX >= x && clickY >= y) {
                if (clickX < x + 200 && clickY < y + ySize()) {
                    mouseDown = true;
        ...
        protected void onTick(GuiCRIMSON gui) {
            this.color = ClickGUI.getInstance().getTheme().buttonColor;
            if (module != null) {
